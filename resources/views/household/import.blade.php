    ...
    <div class="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200/80 overflow-hidden shadow-sm">
            ...
            <div class="bg-[#0e1e3a] px-6 py-5 border-b border-gray-900 text-white flex justify-between items-center">
                <div>
                    <h2 class="text-sm font-bold uppercase tracking-wider">Import Household Member Registry</h2>
                    <p class="text-xs text-blue-200 mt-1">Upload Excel spreadsheet of family member records.</p>
                </div>
            ...
            <div class="p-6 md:p-8 space-y-6">
                ...
                <div class="pt-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <button type="button" id="start-btn" disabled
                        ...
                        <i class="fas fa-play mr-2"></i> Start Households Import
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="bg-blue-50 border border-blue-150 rounded-2xl p-6 space-y-3">
            ...
            <ul class="space-y-2 text-xs text-blue-700 font-medium">
                <li class="flex items-start">
                    ...
                    <span>Columns must align with model properties: UnitNo, userId, firstName, lastName, AdultOrMinor, Relation, Student, Age, FamilySize, CertificationDate, RecertificationDate, Code, dob.</span>
                </li>
            </ul>
        </div>
    </div>

    <!-- Script to parse and progressively import households -->
    <script>
        let parsedRows = [];
        ...
        fileInput.addEventListener('change', function(e) {
            ...
            reader.onload = function(e) {
                try {
                    ...
                    logConsole.innerHTML += `<p class="text-yellow-400">// Ready to seed household members. Click 'Start Households Import' below.</p>`;
                } catch (error) {
                    console.error(error);
        ...
        startBtn.addEventListener('click', async function() {
            ...
            for (let i = 0; i < total; i++) {
                ...
                progressStatus.innerText = `Importing households: ${rowNumber} of ${total}...`;

                try {
                    const response = await fetch('{{ route("household.import.row") }}', {
                        method: 'POST',
                        headers: {
