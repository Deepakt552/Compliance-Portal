    ...
    <div class="space-y-6 max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-2xl border border-gray-200/80 overflow-hidden shadow-sm">
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-150">
                <h1 class="text-sm font-bold text-[#0e1e3a] uppercase tracking-wider">Edit Administrator</h1>
                <p class="text-xs text-gray-400 mt-1">Modify details for: {{ $admin->name }}.</p>
            </div>

            <!-- Error List -->
            @if($errors->any())
                <div class="p-6 bg-red-50 border-b border-red-155 space-y-2">
            ...
            <form action="{{ route('admins.update', $admin->id) }}" method="POST" class="p-6 md:p-8 space-y-6" autocomplete="off">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $admin->name) }}" required autocomplete="off"
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $admin->email) }}" required autocomplete="off"
                               class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-[#0e1e3a] focus:ring focus:ring-[#0e1e3a] focus:ring-opacity-20 text-xs py-2.5">
                    </div>

                    <!-- Password -->
                    <div>
                        <label for="password" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Password (Leave blank to keep existing)</label>
